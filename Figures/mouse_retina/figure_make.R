...
plt = getplots(UMAP_data, make_legend = T, batch_indicator = 'BatchID') + ggtitle(paste0(methods[i], " (HVGs, ARI=", round(tmp$ARI[i], digits = 2), ")")) +
  scale_color_brewer(palette = "Set2")
...
for(i in c(1:length(methods))) {
  link = getlink(methods[i], type = 'LVG')
  UMAP_data = read.csv(link, row.names = NULL)[-1]
  UMAPs4[[i]] = getplots(UMAP_data, batch_indicator = 'BatchID') + ggtitle(" ") +
    scale_color_brewer(palette = "Set2")
}
...
ARIs = read.csv('../mouseretina_embedding/ARIsummary.csv')
ARIs = rbind(ARIs, read.csv('../mouseretina_embedding/ARIsummary_competitors.csv'))[-1]
