...
genespaceARIs = genespaceARIs[genespaceARIs$Method != "Naive HVG",]
...
for(i in c(1:length(methods))) {
  ...
  if(file.exists(link)) {
    UMAP_data = read.csv(link, row.names = NULL)[-1]
    UMAPs4[[i]] = getplots(UMAP_data, batch_indicator = 'Technology') + ggtitle(" ") +
      scale_color_brewer(palette = "Set2")
  } else {
    UMAPs4[[i]] = ggplot() + 
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
ARIs = read.csv('../pancreas_embedding/ARIsummary.csv')
ARIs = rbind(ARIs, read.csv('../pancreas_embedding/ARIsummary_competitors.csv'))[-1]
