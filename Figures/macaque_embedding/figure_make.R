...
plt = getplots(UMAP_data, make_legend = T, batch_indicator = 'Region') + ggtitle(paste0(methods[i], " (ARI=", round(tmp$ARI[i], digits = 2), ")")) +
  scale_color_brewer(palette = "Set2") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))
...
for(i in c(1:length(methods))) {
  link = getlink(methods[i], type = 'HVG')
  UMAP_data = read.csv(link, row.names = NULL)[-1]
  UMAPs3[[i]] = getplots(UMAP_data, batch_indicator = 'Macaque')  + ggtitle(" ") +
    scale_color_brewer(palette = "Set2")
}

plt = getplots(UMAP_data, make_legend = T, batch_indicator = 'Macaque') + ggtitle(paste0(methods[i], " (ARI=", round(tmp$ARI[i], digits = 2), ")")) +
  scale_color_brewer(palette = "Set2") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))
ggsave("type_batch_macaque.tiff", plt, dpi = 300)
...
plt = getplots(UMAP_data, make_legend = T, batch_indicator = 'Sample') + ggtitle(paste0(methods[i], " (ARI=", round(tmp$ARI[i], digits = 2), ")")) +
  scale_fill_manual(values = mycolors2) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))
ggsave("type_batch_sample.tiff", plt, dpi = 300)

full = plot_grid(plotlist = c(UMAPs1, UMAPs2, UMAPs3, UMAPs4), ncol = length(UMAPs1))

ggsave('main.tiff', height = 7.4, width = 8.5 * 5/4, units = 'in', full, dpi = 300)
