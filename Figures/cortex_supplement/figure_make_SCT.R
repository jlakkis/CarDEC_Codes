...
genespaceARIs = read.csv('SCT ARIs.csv')
getlink = function(method) paste0(method, '.csv')
...
getplots = function(data, batch_indicator = NA, make_legend = F){
  ...
  myplot = myplot + theme(legend.position = "none") + theme(plot.title = element_text(hjust=0.5)) + theme(plot.title = element_text(size=6))
  if(make_legend) {
...
plt = getplots(UMAP_data, make_legend = T, batch_indicator = 'Sequencing.Method') + ggtitle(paste0(methods[i], " (ARI=", round(tmp$ARI[i], digits = 2), ")")) +
  scale_color_brewer(palette = "Set2")
ggsave("type_batch.tiff", plt, dpi = 300)

full = plot_grid(plotlist = c(UMAPs1, UMAPs2), ncol = nrow(genespaceARIs))

ggsave('main_SCT.tiff', height = 5 * 1.1/2, width = 7 * 1.1 * 3/5, full, dpi = 300, units = "in")
