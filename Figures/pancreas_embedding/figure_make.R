...
plt = getplots(UMAP_data, make_legend = T, batch_indicator = 'Technology') + ggtitle(paste0(methods[i], " (ARI=", round(tmp$ARI[i], digits = 2), ")")) +
  scale_color_brewer(palette = "Set2") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))
...
ggsave('main.tiff', height = 7.4/2, width = 8.5/2 * 3/2, units = 'in', full, dpi = 300)
