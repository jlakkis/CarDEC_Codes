...
for(i in c(1:length(methods))) {
  link = getlink(methods[i], type = 'HVG')
  UMAP_data = read.csv(link, row.names = NULL)[-1]
  UMAPs2[[i]] = getplots(UMAP_data, batch_indicator = 'Macaque')  + ggtitle(" ") +
    scale_color_brewer(palette = "Set2")
}

plt = getplots(UMAP_data, make_legend = T, batch_indicator = 'Macaque') + ggtitle(paste0(methods[i], " (HVGs, ARI=", round(tmp$ARI[i], digits = 2), ")")) +
  scale_color_brewer(palette = "Set2") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))
...
for(i in c(1:length(methods))) {
  link = getlink(methods[i], type = 'LVG')
  UMAP_data = read.csv(link, row.names = NULL)[-1]
  UMAPs4[[i]] = getplots(UMAP_data, batch_indicator = 'Macaque') + ggtitle(" ") +
    scale_color_brewer(palette = "Set2")
}

full = plot_grid(plotlist = c(UMAPs1, UMAPs2,UMAPs3, UMAPs4), ncol = sum(genespaceARIs$Type == 'HVG'))

ggsave('main.tiff', height = 5 * 1.1, width = 7 * 1.1, units = 'in', full, dpi = 300)

ARIs = read.csv('../macaque_embedding/ARIsummary.csv')
ARIs = rbind(ARIs, read.csv('../macaque_embedding/ARIsummary_competitors.csv'))[-1]
...
ggsave('embeddingplot.tiff', height = 3.65, width = 7.35, units = "in", myplot, dpi = 300)
