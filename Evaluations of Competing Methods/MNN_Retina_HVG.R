...
path = "../Data/retina_to_mingyao"
mydata = read_retina(path)
...
hvgs = get_hvgs(mydata@assays$RNA@data, meta_data$BatchID, rownames(mydata@assays$RNA@data))
...
write.csv(output@assays@data$corrected, paste0(path, "/corrected_data_Retina.csv"))
write.csv(meta_data, paste0(path, "/corrected_metadata_Retina.csv"))